...
# logging: detailed file, silent console (we'll print only final short message)
...
# small helpers for stage output (logged)
...
AUTH_RECORD_ID = os.getenv("AUTH_RECORD_ID")  # only ID required

QUALYS_URL = "https://qualysapi.qualys.com/api/2.0/fo/asset/group/"
# auth update must use unix type (per request)
...
# Stage 6 - update existing unix auth record (only ID required)
if AUTH_RECORD_ID and combined_ips:
    ...
    try:
        ...
        # try parse response text for confirmation
        try:
        ...
        logger.info("Added %d IPs to Auth Record ID %s", len(combined_ips), AUTH_RECORD_ID)
        stage_success(stage_name)
    except Exception as e:
        overall_ok = False
        stage_fail(stage_name, e)
