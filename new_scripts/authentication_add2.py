...
# NOTE: Change 'unix' to 'windows', 'cisco', etc., depending on your target record type.
QUALYS_URL = "https://qualysapi.qualys.com/api/2.0/fo/auth/unix/"

# ---------------------------------
# Validation
# ---------------------------------
if not all([QUALYS_USERNAME, QUALYS_PASSWORD, AUTH_RECORD_ID, TARGET_IPS]):
    error_msg = "Missing required environment variables (.env). Check credentials, ID, and IPs."
...
logging.info(f"Target Record ID: {AUTH_RECORD_ID} | IPs: {formatted_ips}")
...
payload = {
    "action": "update",
    "id": AUTH_RECORD_ID,
    # Using 'add_ips' appends to the record. If you want to overwrite completely, use 'ips'.
...
try:
    response = requests.post(
        ...
        timeout=30     # Prevents the script from hanging forever
    )
